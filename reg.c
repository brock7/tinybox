...
struct string_table str_table[] = {
	{ STRING_USAGE, L"The syntax of this command is:\n\nREG [ ADD | DELETE | QUERY ]\nREG command /?\n" }, 
	{ STRING_ADD_USAGE, L"REG ADD key_name [/v value_name | /ve] [/t type] [/s separator] [/d data] [/f]\n" }, 
	{ STRING_DELETE_USAGE, L"REG DELETE key_name [/v value_name | /ve | /va] [/f]\n" }, 
	{ STRING_QUERY_USAGE, L"REG QUERY key_name [/v value_name | /ve] [/s]\n" }, 
	{ STRING_SUCCESS, L"The operation completed successfully\n" }, 
	{ STRING_INVALID_KEY, L"Error: Invalid key name\n" }, 
	{ STRING_INVALID_CMDLINE, L"Error: Invalid command line parameters\n" }, 
	{ STRING_NO_REMOTE, L"Error: Unable to add keys to remote machine\n" }, 
	{ STRING_CANNOT_FIND, L"Error: The system was unable to find the specified registry key or value\n" }, 
};

static int reg_message(int msg)
{
    static const WCHAR formatW[] = {'%','s',0};
    WCHAR msg_buffer[8192] = {0};
	int i = 0;
	for (; i < sizeof(str_table) / sizeof(str_table[0]); i ++) {
		if (str_table[i].id == msg) {
			wcscpy(msg_buffer, str_table[i].str);
			break;
		}
	}
...
int reg_wmain(int argc, WCHAR *argvW[])
{
    int i;
